...
import org.eclipse.ocl.examples.pivot.validation.LoadableConstraintDescriptor.UML;
...
//		addParams.add(getCompToAddSVP());
		...
		if(umlCompToAdd != null){
			setCompToAddSVP(Manager.createSingleValueParameter(OclStringManager.getComponentQuery(umlCompToAdd)));
			addParams.add(getCompToAddSVP());
		}
...
//		addParams.add(getDeployedCompsMVP());
		...
		setDeployedCompsMVP(Manager.createMultipleValuedParameter(OclStringManager.getAllDeployedElementsQuery(getUmlTargetNodes())));
		addParams.add(getDeployedCompsMVP());
...
//		addPreAnd.getArguments().add(addPreAndNot);
	...
	public void createPostCondition() {
		...
		//TODO getDeployedComps è l'insieme delle Component deployate sui target Nodes. Non va bene: la condition dice che 
		// su ogni target Node ci deve stare la Component aggiunta. Con l'unione basta che stia su uno dei target...
...
//				.createExistsOperator(Manager.createMultipleValuedParameter(OclStringManager.getAllNodesQuery()));
//		addPostAndForallTargetsAnd.getArguments().add(addPostAndForallTargetsExists);
//		ExistsOperator addPostAndExistsInD = Manager.createExistsOperator(getCompToAddSVP(),
